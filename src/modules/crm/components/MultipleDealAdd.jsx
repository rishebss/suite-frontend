...
const MultipleDealAdd = ({ isOpen, onClose, dealIds, onAdded, currentPipelineId }) => {
  ...
  const handleAdd = async () => {
    if (!selectedPipeline || !dealIds?.length) return;
    setIsAdding(true);
    setError('');
    ...
    try {
      ...
      setError(data?.detail || data?.error || data?.pipeline_id?.[0] || 'Failed to add deals.');
    } finally {
      setIsAdding(false);
    }
  };
            ...
            dealIds.length >= 500 ? (
              <div className="w-full space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-white/40">Adding deals...</span>
...
export default MultipleDealAdd;
