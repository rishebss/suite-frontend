...
import { X, Loader2, Check, Plus, Layout } from 'lucide-react';
import { BiSolidAddToQueue } from 'react-icons/bi';
import axios from 'axios';
import { cn } from '@/lib/utils';

const SingleDealAdd = ({ isOpen, onClose, deal, onAdded }) => {
  ...
  const handleAdd = async () => {
    ...
    setError('');
    try {
      // Create a NEW CRM entry in the target pipeline via custom endpoint
      await axios.post(`/api/crm/pipeline/${deal.id}/copy-to-pipeline/`, {
        pipeline_id: selectedPipeline.id,
      });

      onAdded?.(selectedPipeline.id);
      onClose();
    } catch (err) {
      const data = err.response?.data;
      setError(data?.detail || data?.contact?.[0] || data?.pipeline?.[0] || 'Failed to add deal.');
    } finally {
      setIsAdding(false);
    }
  };
      ...
      <div className="relative w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[70vh]">
        <div className="px-6 py-5 border-b border-zinc-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-medium text-white uppercase tracking-wider">Add to Pipeline</h2>
        ...
        <div className="px-6 py-4 border-t border-zinc-800 bg-black/50 flex justify-end gap-3 shrink-0">
          ...
          <button onClick={handleAdd} disabled={isAdding || !selectedPipeline || isCreatingPipeline}
            ...
            {isAdding ? <><Loader2 size={12} className="animate-spin" /> Adding...</> : 'Add'}
          </button>
        </div>
      </div>
    </div>,
